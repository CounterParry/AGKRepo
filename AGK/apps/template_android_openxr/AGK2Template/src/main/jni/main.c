...
//static int done     =  0; // Replaced By: eAppStatus eStatus = Stage_1_Not_Init_Status;
...
{        
	...
	enum eAppStatus eStatus = getopenxrstatus();

 	if (eStatus == Stage_2_OpenXR_Init_Status)
    {
    	LOGI( "Initialising" );
    	init( &data );
		setopenxrstatus(Stage_3_AGK_Active_Status);
    }
    else if (eStatus >=  Stage_3_AGK_Active_Status)
    {
    	LOGI( "Updating" );
		updateptr( &data );
    	updatesize();
...
//int initialised = 0; // Replace By eInitialised eInit = Not_Initialised
...
{
	if ( /*initialised == 0*/ getwindowinitialised() == Not_Initialised) return 0;
...
void android_main(struct android_app* state)
{
    ...
    while (1)
	{
		...
		switch (eStatus)
		{
			...
			case Stage_2_OpenXR_Init_Status:   LOGI("Stage 2: OPenXR Init");    break;
			case Stage_3_AGK_Active_Status:    LOGI("Stage 3: AGK Active");     break;
			case Stage_4_OpenXR_Active_Status: LOGI("Stage 4: OpenXR Active");  break;
			case Shutdown_Status:              LOGI("Stage ?: Shutdown");       break;
			case Failed_Status:                LOGI("Stage ?: Failed");         break;
			default: LOGI("WHAT? (1)"); break;
		}

		switch (eMode)
		{
			case Not_Active_Mode: LOGI("Its not active time...");  break;
    		case Go_Mode:         LOGI("Its go time...");          break;
    		case Paused_Mode:     LOGI("Its paused time...");      break;
    		case Lost_Focus_Mode: LOGI("Its lost time...");        break;
			default: LOGI("WHAT? (2)"); break;
		}

		switch (eInit)
		{
			case Not_Initialised:            LOGI("Not INIT");          break;
		...
		*/
		...
		if (eMode == Go_Mode)
		{
			...
			if (eStatus >= Stage_4_OpenXR_Active_Status)
			{
			...
			else if (eStatus == Stage_2_OpenXR_Init_Status && eInit == Window_Initialised)
			{
				engine_init_display(&engine);
				resumeapp2();
				engine.animating = 1;
			}
			else if (eStatus == Stage_1_Not_Init_Status && eInit == Window_Initialised)
			{ 
				initopenxr_c(&engine);
			}
			else
		...
		if (eStatus == Shutdown_Status || eStatus == Failed_Status ) break;
    }
